...
console.log('userId --->', userId);
  const userData = await (await fetch(`http://localhost:3001/${userId}`)).json();
...
router.delete('/:listId', async (req, res) => {
  ...
  const userId = req.user.id;
  const response = await fetch(`http://localhost:3001/${userId}/${listId}`, {
    headers: { 'Content-Type': 'application/json' },
    method: 'DELETE',
  });
...
console.log('userId --->', userId);
  ...
  const response = await fetch(`http://localhost:3001/${userId}/${listId}/${entryName}`, {
