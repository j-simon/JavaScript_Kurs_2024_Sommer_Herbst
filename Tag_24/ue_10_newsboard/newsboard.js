...

{
  ...
  const init = () => {
    ...
    $("span.message_number").innerHTML = messages.length // 3 in die Bubble
    ...
    $('body').addEventListener("keyup", (e) => {
      // aus console.log() : key = "ArrowRight"
      // aus console.log() : key = "ArrowLeft"
      if (e.key === "ArrowRight") 
        if (!e.altKey)
          nextMessage(e)
        else
        ...
        console.log("body key-up entdeckt! ", e)
      })
  ...
  const nextMessage = e => {
    ...
    e.preventDefault() // vebiete Standardverhalten, e ist hier ein a-tag
  }
  const prevMessage = e => {
    showMessageByNumber((currentMessageNumber -= 1))
    e.preventDefault()
  ...
  const showMessageByNumber = messageNumber => {
    // $("span.message_number").innerHTML = currentMessageNumber 
